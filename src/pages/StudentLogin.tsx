...
import { BookOpen, Mail, Lock, Eye, EyeOff, ArrowRight, Loader2, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function StudentLogin() {
  ...
  const validateEmail = (val: string): string => {
    ...
    if (lower.endsWith("@vitstudent.ac.in")) return "";
    return "Please use your student email (name.year@vitstudent.ac.in)";
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-20 right-20 w-96 h-96 bg-cyan-500/15 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: "1s" }} />
              ...
              <div className="glass-card-elevated p-8 rounded-3xl">
                ...
                <form onSubmit={handleLogin} className="space-y-5">
                  <div className="space-y-2">
                    <Label className="text-gray-300 text-sm">Student Email</Label>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-blue-400" />
                      <Input type="email" value={email} onChange={e => { setEmail(e.target.value); setEmailError(""); setError(""); }} onBlur={handleEmailBlur} placeholder="name.year@vitstudent.ac.in" className={`pl-10 bg-white/5 border-white/10 text-white placeholder:text-gray-500 focus:border-blue-500/50 focus:ring-blue-500/20 rounded-xl h-12 ${emailError ? "border-red-500/60" : ""}`} />
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input type={showPassword ? "text" : "password"} value={password} onChange={e => { setPassword(e.target.value); setError(""); }} placeholder="Enter your password" className="pl-10 pr-10 bg-white/5 border-white/10 text-white placeholder:text-gray-500 focus:border-blue-500/50 focus:ring-blue-500/20 rounded-xl h-12" />
                  ...
                  <div className="flex items-center justify-between">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <Checkbox checked={rememberMe} onCheckedChange={(v) => setRememberMe(!!v)} className="border-white/20 data-[state=checked]:bg-blue-500 data-[state=checked]:border-blue-500" />
                      <span className="text-xs text-gray-400">Remember me</span>
                    </label>
                    <button type="button" onClick={() => { setView("forgot"); setError(""); setEmailError(""); }} className="text-xs text-blue-400 hover:text-blue-300 transition-colors">Forgot password?</button>
                  </div>
                  <button type="submit" disabled={isLoginDisabled} className="w-full h-12 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-semibold hover:scale-105 transition-all duration-300 shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2 group disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed">
                ...
                <div className="mt-6 pt-5 border-t border-white/10 text-center">
                  <Link to="/faculty-login" className="text-sm text-gray-400 hover:text-white transition-colors">Are you faculty? <span className="text-blue-400 hover:text-blue-300">Login here →</span></Link>
                </div>
              </div>
            </motion.div>
          )}
          ...
          {view === "forgot" && (
            <motion.div key="forgot" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="glass-card-elevated p-8 rounded-3xl">
                ...
                <p className="text-gray-400 text-sm mb-6">Enter your student email to receive a reset link</p>
                <form onSubmit={handleForgotPassword} className="space-y-5">
                  <div>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-blue-400" />
                      <Input type="email" value={forgotEmail} onChange={e => { setForgotEmail(e.target.value); setEmailError(""); }} placeholder="name.year@vitstudent.ac.in" className={`pl-10 bg-white/5 border-white/10 text-white placeholder:text-gray-500 rounded-xl h-12 ${emailError ? "border-red-500/60" : ""}`} />
                    </div>
                    {emailError && <p className="text-red-400 text-xs mt-1">{emailError}</p>}
                  </div>
                  <button type="submit" className="w-full h-12 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-semibold hover:scale-105 transition-all duration-300 shadow-lg">Send Reset Link</button>
            ...
            <motion.div key="change" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="glass-card-elevated p-8 rounded-3xl">
                ...
                <form onSubmit={handleChangePassword} className="space-y-4">
                  ...
                  <button type="submit" disabled={!oldPass || !newPass || !confirmPass || loading} className="w-full h-12 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-semibold hover:scale-105 transition-all duration-300 shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed">
